  ...
  try {
    ...
    function logStack(prefix) {
      try {
        ...
        (origWarn || console.log).call(
          console,
          "[qwik-warn-debug] stack:\n" + (e.stack || "no stack"),
        );
      } catch {}
    }
    console.warn = function () {
      try {
        ...
        if (msg.includes("QWIK WARN") && msg.includes("unsupported value was passed to the JSX")) {
          (origWarn || console.log).apply(
            console,
            ["[qwik-warn-debug]"].concat([].slice.call(arguments)),
          );
          ...
          try {
            ...
            var hint = active
              ? active.tagName +
                (active.id ? "#" + active.id : "") +
                (active.className ? "." + String(active.className).trim().replace(/\s+/g, ".") : "")
              : "";
            (origWarn || console.log).call(
              console,
              "[qwik-warn-debug] route:",
              path,
              "active:",
              hint,
            );
