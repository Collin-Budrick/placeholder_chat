...
export default component$(() => {
  ...
  useVisibleTask$(() => {
    ...
    const init = () => {
      ...
      if (!wrapper) { requestAnimationFrame(() => { init(); }); return; }
      ...
      const cancelAnim = () => { if (animId != null) cancelAnimationFrame(animId); animId = null; };
      ...
      const onKey = (ev: KeyboardEvent) => {
        ...
        if (t !== null) { ev.preventDefault(); animateTo(t); }
      };
      ...
      const onDocClick = (ev: Event) => {
        ...
        ev.preventDefault();
        try { const top = el.getBoundingClientRect().top + wrapper.scrollTop; animateTo(top); } catch {}
        try { history.pushState({}, '', `#${id}`); } catch {}
      };

      try { wrapper.setAttribute('data-smooth', 'active'); } catch {}
      ...
      (window as any).__smooth_cleanup = () => {
        ...
        try { wrapper.removeAttribute('data-smooth'); } catch {}
      };
    };
