  ...
  const start$ = $(async () => {
    ...
    try {
      ...
      const onEnter = () => {
        try {
          a([
            [el, { transform: 'scale(1.04)' }, { duration: 0.12, easing: 'ease-out' }],
            [el, { transform: 'scale(1.0)' }, { duration: 0.18, easing: 'ease-out' }],
          ]);
        } catch { /* ignore */ }
      };
      ...
      const onClick = () => {
        try {
          ...
          const wrap = wrapperRef.value || (el.parentElement as HTMLElement | null);
          if (!wrap) return;
          const count = 16;
          for (let i = 0; i < count; i++) {
            const spark = document.createElement('div');
            spark.setAttribute('aria-hidden', 'true');
            spark.className = 'pointer-events-none absolute rounded-full shadow';
            ...
            a([
              [spark, { transform: `translate(-50%, -50%) translate(${dx}px, ${dy}px) scale(1)` }, { duration: outDur, easing: 'ease-out' }],
              [spark, { transform: `translate(-50%, -50%) translate(${dx}px, ${dy}px) scale(${1.6 + Math.random() * 0.6})`, opacity: 0, filter: 'blur(2px)' }, { duration: fadeDur, easing: 'ease-in' }],
            ]).finished.finally(() => { spark.remove(); }).catch(() => spark.remove());
          }
        } catch { /* ignore */ }
      };
