...
import { $, component$, isServer, useOn, useSignal, useTask$ } from "@builder.io/qwik";
...
const TypeTitle = component$((props: Props) => {
  ...
  const startTyping = $((): void => {
    ...
    const storage: Storage | null = (() => {
      try {
        return storageKind === "session" ? globalThis.sessionStorage : globalThis.localStorage;
      } catch {
        return null;
      }
    })();
    ...
    if (props.resetOnReload && storage && cacheKey) {
      try {
        ...
        if (perf?.getEntriesByType) {
          ...
          const actStart = (nav as unknown as { activationStart?: number })?.activationStart ?? 0;
          isReload =
            !!nav &&
            (nav.type === "reload" ||
              (nav.type === "navigate" && nav.transferSize === 0 && actStart > 0));
  ...
  useTask$(({ track }) => {
    ...
    if (sup) {
      ...
      try {
        const storageKind = props.cacheStorage ?? "local";
        const storage: Storage | null =
          storageKind === "session" ? globalThis.sessionStorage : globalThis.localStorage;
        const mode = props.cache ?? "route";
        if (mode !== false) {
          const key =
            props.cacheKey ??
    ...
    <h1 class={cn(props.class)} aria-label={props.text} ref={elRef}>
      ...
      {props.showCaret !== false && (
        <span
          ...
          style={{
            animation: phase.value === "idle" ? "caret-blink 0.8s steps(1,end) infinite" : "none",
            willChange: "opacity",
          }}
          aria-hidden="true"
        />
