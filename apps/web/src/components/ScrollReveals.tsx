import { component$, useTask$, isServer, $ } from "@builder.io/qwik";
  ...
  try {
    ...
    const io = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        ...
        (async () => {
          try {
                const player = await animateMotion(el, { y: [16, 0], opacity: [0, 1] }, { duration: 0.5, easing: 'cubic-bezier(.22,.9,.37,1)' } as any);
            if (player) players.push(player);
            try { if (player && (player as any).finished) await (player as any).finished; } catch { /* ignore */ }
          } catch { /* ignore */ }
        })();
    ...
    return () => {
      try { observers.forEach(o => o.disconnect()); } catch { /* ignore */ }
      try { players.forEach(p => p?.cancel?.()); } catch { /* ignore */ }
    };
  } catch {
    return () => {};
  }
...
export const ScrollReveals = component$(() => {
  useTask$(() => {
    ...
    if (idleCb) {
      idleId = idleCb(() => { void start(); });
    } else {
      timeoutId = window.setTimeout(() => { void start(); }, 200);
    }
