import { component$, useSignal, useOnWindow, $, useTask$ } from '@builder.io/qwik';
import { Link, useLocation } from '@builder.io/qwik-city';
import { LuHome, LuMail, LuSearch, LuUser, LuInfo } from '@qwikest/icons/lucide';
...
export default component$(() => {
  ...
  useOnWindow('scroll', $(() => {
    try {
      scrolled.value = (globalThis as any).scrollY > 8;
    } catch { /* ignore */ }
  }));
  ...
  return (
    <nav
      class={[
        ...
        scrolled.value ? 'translate-y-[-2px] scale-[.995]' : 'translate-y-0 scale-100',
        ...
        <div class="navbar-center w-full">
          <ul class="grid w-full grid-cols-5 place-items-center gap-0 px-2 relative">
            <li class="w-full grid place-items-center">
              <Link aria-current={isActive('/') ? 'page' : undefined} href="/" class="px-2 py-1.5 rounded-lg transition-colors">
                <LuHome class="w-6 h-6" />
                <span class="sr-only">Home</span>
              </Link>
            </li>
            <li class="w-full grid place-items-center">
              <Link aria-current={isActive('/about') ? 'page' : undefined} href="/about" class="px-2 py-1.5 rounded-lg transition-colors">
                <LuInfo class="w-6 h-6" />
                <span class="sr-only">About</span>
              </Link>
            </li>
            <li class="w-full grid place-items-center">
              <Link aria-current={isActive('/contact') ? 'page' : undefined} href="/contact" class="px-2 py-1.5 rounded-lg transition-colors">
                <LuMail class="w-6 h-6" />
                <span class="sr-only">Messages</span>
              </Link>
            </li>
            <li class="w-full grid place-items-center">
              <Link aria-current={isActive('/profile') ? 'page' : undefined} href="/profile" class="px-2 py-1.5 rounded-lg transition-colors">
