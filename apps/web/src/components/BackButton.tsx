...
const backNav = $((ev: Event, elParam?: Element) => {
  try {
    ...
    const el =
      (elParam as Element | undefined) || current || (tgt?.closest?.("button") as Element | null);
...
const BackButton = component$((props: BackButtonProps) => {
  ...
  const onPress = $((ev: Event) => {
    try {
      ...
      else if (t === "pointerup" || t === "pointerleave" || t === "blur" || t === "keyup")
  ...
  const onPreArm = $((ev: Event) => {
    try {
      ...
      const dsTarget = el?.getAttribute ? String(el.getAttribute("data-target") || "") : "";
