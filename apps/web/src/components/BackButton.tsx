...
const backNav = $((ev: Event, elParam?: Element) => {
	try {
		...
		const el =
			(elParam as Element | undefined) ||
			current ||
			(tgt?.closest?.("button") as Element | null);
...
const BackButton = component$((props: BackButtonProps) => {
			...
			try {
				...
				if (!animating.value) {
					...
					const step = () => {
						...
						const still =
							Math.abs(targetX.value - curX.value) < 0.5 &&
							Math.abs(targetY.value - curY.value) < 0.5 &&
							Math.abs(targetO.value - curO.value) < 0.01;
						if (still) {
							// Snap to final to avoid lingering decimals and stop
							overlay.style.setProperty(
								"--mx",
								`${targetX.value.toFixed(2)}px`,
							);
							overlay.style.setProperty(
								"--my",
								`${targetY.value.toFixed(2)}px`,
							);
	...
	const goExplicit = $(async (ev: Event, elParam?: Element) => {
		try {
			...
			const el =
				...
				current ||
				(tgt?.closest?.("button") as Element | null);
			const dsTarget = String(
				(() => {
					try {
						return el?.getAttribute ? el.getAttribute("data-target") : "";
					} catch {
						return "";
					}
				})() || "",
	...
	const onPreArm = $((ev: Event) => {
		try {
			const el =
				(ev?.currentTarget as Element | null) || (ev?.target as Element | null);
			const dsTarget = el?.getAttribute
	...
	return (
		<button
			...
			class={cn(
				...
				sizeCls,
				"text-base-content/70 hover:text-base-content/90 items-center justify-center rounded-full transition-all focus:outline-none",
				props.class,
			)}
			ref={(el) => {
			...
			onKeyDown$={$((ev) => {
				onPress(ev);
				const e = ev as KeyboardEvent;
				if (e.key === "Enter" || e.key === " ") onPreArm(ev);
			})}
			onKeyUp$={onPress}
			data-fallback={
				props.fallbackHref ?? (isSignup ? "/login" : isLogin ? "/" : "/")
			}
		...
		>
			<svg
				...
				style={{
					transform: backPressed.value ? "scale(0.9)" : "scale(1)",
					willChange: "transform",
				}}
			>
				<path
					d="M15.5 4.5L8.5 12l7 7.5"
					stroke="currentColor"
					stroke-width="3.75"
			...
			<span
				aria-hidden="true"
				class="pointer-events-none absolute -inset-2 z-0 rounded-full opacity-0 transition-opacity group-focus-visible:opacity-100"
				style={{
					background:
						"conic-gradient(from 220deg at 50% 50%, rgba(203,213,225,0.95), rgba(148,163,184,0.95), rgba(203,213,225,0.95))",
					WebkitMaskImage:
			...
			<span
				aria-hidden="true"
				class={`pointer-events-none absolute inset-0 z-0 rounded-full transition-opacity ${backPressed.value ? "opacity-100" : "opacity-0"}`}
				style={{
					background: "#E9D5FF",
					border: "1px solid #D6BCFA",
					boxShadow:
						"inset 0 3px 14px rgba(0,0,0,.20), inset 0 -3px 8px rgba(0,0,0,.14)",
				}}
			/>
			...
			<span
				aria-hidden="true"
				ref={(el) => {
					backOverlay.value = el as HTMLElement;
				}}
				class="pointer-events-none absolute inset-0 z-0 rounded-full opacity-0 shadow-sm transition-opacity"
				style={{
					background: "rgba(233,213,255,0.45)",
					border: "1px solid #D6BCFA",
					WebkitMaskImage:
						"radial-gradient(48px 48px at var(--mx) var(--my), rgba(0,0,0,0.85) 0%, rgba(0,0,0,0.55) 52%, rgba(0,0,0,0.18) 82%, transparent 100%)",
					maskImage:
						"radial-gradient(48px 48px at var(--mx) var(--my), rgba(0,0,0,0.85) 0%, rgba(0,0,0,0.55) 52%, rgba(0,0,0,0.18) 82%, transparent 100%)",
					willChange: "opacity",
				}}
