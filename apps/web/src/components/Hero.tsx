...
export const Hero = component$(() => {
	...
	useTask$(() => {
		...
		try {
			...
			if (
				(savedMsg && typeof savedMsg === "string") ||
				Number.isFinite(savedIdx)
			) {
				// Prefer exact index if valid; else derive from message
				if (
					Number.isFinite(savedIdx) &&
					savedIdx >= 0 &&
					savedIdx < HERO_LINES.length
				) {
					idx.value = savedIdx as number;
					titleText.value =
						HERO_LINES[idx.value] ?? savedMsg ?? titleText.value;
				} else if (savedMsg) {
	...
	useTask$(() => {
		...
		try {
			typedOnce.value = !!globalThis.sessionStorage?.getItem("typedOnce");
		} catch (_e) {
			void _e;
		}
			...
			try {
				if (h1Ref.value && pRef.value && ctaRef.value) {
					try {
						...
						const tl = (await timelineMotion(items)) as
		...
		return () => {
			cancelled = true;
			try {
				if (idleCbId !== undefined && (window as W).cancelIdleCallback) {
					(window as W).cancelIdleCallback?.(idleCbId);
				}
			} catch (_e) {
				void _e;
			}
	...
	const scheduleErase$ = $(() => {
		...
		if (eraseTimerId.value) {
			try {
				globalThis.clearTimeout(eraseTimerId.value);
			} catch {
				/* ignore */
			}
		...
		const ms = Math.max(
			600,
			(titleText.value?.length || 0) * typingSpeed + dwell,
		);
		const id = globalThis.setTimeout(() => {
			eraseKey.value = Date.now();
		...
		<section ref={rootRef} class="relative w-full overflow-hidden">
			...
			<div aria-hidden class="pointer-events-none absolute -inset-40">
				<div
					...
					style={{
						background:
							"radial-gradient(closest-side, rgba(199,183,255,0.12), rgba(199,183,255,0.05), transparent)",
					}}
				/>
			</div>
			<div class="relative mx-auto max-w-6xl px-6 py-24 text-center">
				...
				<div class="mt-8 flex items-center justify-center gap-4">
					<a ref={ctaRef} href="#get-started" class="btn btn-primary">
						Get Started
					</a>
