...
try {
	const certFile = (Bun.env.TLS_CERT_FILE || Bun.env.SSL_CERT_FILE) as
		| string
		| undefined;
	const keyFile = (Bun.env.TLS_KEY_FILE || Bun.env.SSL_KEY_FILE) as
		| string
		| undefined;
	if (certFile && keyFile) {
	...
	async fetch(request: Request) {
		...
		const forwardedHttps = (() => {
			try {
				...
				const xfProto =
					h.get("x-forwarded-proto") || h.get("x-forwarded-protocol");
				const forwarded = h.get("forwarded");
				const xfSsl = h.get("x-forwarded-ssl");
				return !!(
					(xfProto && /https/i.test(xfProto)) ||
					(forwarded && /proto=https/i.test(forwarded)) ||
					(xfSsl && /on/i.test(xfSsl))
				);
			} catch {
				return false;
			}
		...
		if (
			!isHttps &&
			(Bun.env.FORCE_HTTPS_REDIRECT === "1" || Bun.env.ENFORCE_TLS === "1")
		) {
			url.protocol = "https:";
...
if (
	tls &&
	(Bun.env.ENABLE_HTTP_REDIRECT === "1" || Bun.env.HTTP_REDIRECT_PORT)
) {
	const redirectPort = Number(
		Bun.env.HTTP_REDIRECT_PORT || (port === 443 ? 80 : port + 1),
	);
	Bun.serve({
		port: redirectPort,
		fetch(req: Request) {
			try {
				...
				return new Response(null, {
					status: 308,
					headers: { Location: `https://localhost:${port}/` },
				});
			}
		},
	});
	console.log(
		`HTTP->HTTPS redirect server on http://localhost:${redirectPort}/`,
	);
}
