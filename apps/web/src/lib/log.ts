...
export function redactHeaders(
	h?: Headers | Record<string, string> | null,
 ...
 */
export function previewBodyMaybe(body: unknown): {
	preview: string;
	size: number;
} {
	...
	try {
		...
		const cloned = JSON.parse(
			JSON.stringify(body, (_k, v) => {
				if (_k && SENSITIVE_BODY_KEYS.includes(String(_k).toLowerCase()))
					return "[REDACTED]";
				return v;
			}),
		);
		...
		if (!entry.env)
			entry.env =
				(typeof process !== "undefined" &&
					((process.env.NODE_ENV ?? process.env?.AUTH_TRUST_HOST)
						? "development"
						: "production")) ||
				"development";
