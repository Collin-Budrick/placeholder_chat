...
export const useUsersLoader = routeLoader$<{
  ...
  if (!res.ok) {
    return {
      error: (payload && (payload.message || payload.error)) ?? `Failed to load (status ${res.status})`,
      status: res.status,
    };
  }
...
export default component$(() => {
  ...
  const confirmDelete = $(async () => {
    ...
    try {
      ...
      if (res.status === 204) {
        ...
        const msg = (await res.json().catch(() => null))?.message ?? `Delete failed (${res.status})`;
        alert(msg);
      }
    } finally {
      busy.value = null;
  ...
  return (
    <main class="min-h-screen p-6">
      <div class="bg-base-content/5 backdrop-blur rounded-xl border border-base-content/10 p-6 w-full max-w-none mx-auto">
        <h2 class="text-2xl font-semibold text-center mb-2">All Users</h2>
        <p class="text-sm text-center text-base-content/70 mb-4">Manage accounts</p>
      ...
      {deleteOpen.value && (
        <div class="fixed inset-0 flex items-center justify-center z-50">
          <div class="absolute inset-0 bg-base-content/50" onClick$={() => closeDelete()} />
          <div class="bg-base-100 rounded-lg p-6 z-60 w-full max-w-md">
            <h3 class="text-lg font-semibold mb-2">Delete user</h3>
            <p class="mb-3">Are you sure you want to delete <b>{deleteEmail.value}</b>? This action cannot be undone.</p>
            <div class="flex justify-end gap-2">
              <button class="btn" onClick$={() => closeDelete()}>Cancel</button>
              <button class="btn btn-error" onClick$={() => confirmDelete()} disabled={busy.value === deleteId.value}>
...
export const head: DocumentHead = {
  ...
  meta: [
    { name: 'description', content: 'Admin dashboard to view, promote, demote, and delete users in Stack.' },
  ],
};
