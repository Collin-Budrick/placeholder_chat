...
export default component$(() => {
  ...
  useVisibleTask$(async () => {
    const from = (globalThis?.location?.pathname || '/profile') + (globalThis?.location?.search || '');
    try {
      ...
      if (res.ok) {
        ...
        try { await logApi({ phase: 'response', url: '/api/auth/me', status: res.status, client: { path: from }, message: 'profile: session ok' }); } catch {}
        show.value = true;
        try { sessionStorage.removeItem('postLogin'); } catch {}
        return;
      }
      // Not authorized
      try { await logApi({ phase: 'error', url: '/api/auth/me', status: res.status, client: { path: from }, message: 'profile: unauthorized, redirect to login' }); } catch {}
    } catch (e: any) {
      try { await logApi({ phase: 'error', url: '/api/auth/me', client: { path: from }, message: 'profile: session check failed ' + String(e?.message || e) }); } catch {}
    }
    // Redirect to login with callback
    const to = '/login?callbackUrl=' + encodeURIComponent(from);
    try { await logApi({ phase: 'request', url: to, client: { path: from }, message: 'redirect: profile -> login' }); } catch {}
  ...
  return (
    <main class="min-h-screen flex items-center justify-center p-6">
      ...
      <AuthCard title="Profile">
        <h3 class="text-xl font-semibold text-center mb-2">Hi {displayName.value}</h3>
        ...
        <button
          ...
          onClick$={$ (async () => {
            try {
              ...
              await fetch('/api/auth/logout', { method: 'POST', headers: { ...csrfHeader() }, credentials: 'same-origin' });
              try { await logApi({ phase: 'request', url: '/login', message: 'redirect: logout -> login' }); } catch {}
            } catch (err) {
              // best-effort: ignore network errors and proceed to clear Auth.js session
              console.debug('backend logout failed', err);
            }
            // Navigate to login
            try { await nav('/login'); } catch { try { location.assign('/login'); } catch {} }
          })}
        >
          Logout
        </button>
