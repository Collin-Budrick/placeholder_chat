      ...
      {enableSmooth ? (
        <SmoothScrollProvider client:idle>
          <main id="content" class="edge-fades flex-1 overflow-auto">
            <div id="scroll-inner" class="grid min-h-full place-items-center pb-24">
              <Slot />
...
export const onStaticGenerate = async () => {
  const base = ["/", "/about", "/contact", "/integrations", "/login", "/signup"];
  try {
    ...
    if (file) {
      try {
        ...
        if (Array.isArray(arr) && arr.length > 0) {
          return {
            routes: Array.from(new Set(arr.map((s: string) => (s && s[0] !== "/" ? `/${s}` : s)))),
          };
        }
      } catch {}
    }
