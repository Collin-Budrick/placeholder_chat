...
function sseBroadcast(event: string) {
  ...
  for (const w of Array.from(sseClients)) {
    w.write(buf).catch(() => { try { w.close(); } catch {}; sseClients.delete(w); });
  }
}
function wsBroadcast(kind: string) {
  for (const ws of Array.from(wsClients)) {
    try { ws.send(kind); } catch { try { ws.close(); } catch {}; wsClients.delete(ws); }
  }
}
setInterval(() => {
...
const server = Bun.serve({
  websocket: {
    open(ws) { try { wsClients.add(ws); } catch {} },
    close(ws) { try { wsClients.delete(ws); } catch {} },
    message(_ws, _msg) { /* server push only */ },
  } as any,
  port,
  hostname,
  async fetch(req) {
    ...
    if (liveReload && pathname === '/__ssg-ws') {
      try { if ((server as any).upgrade?.(req)) return new Response(null, { status: 101 }); } catch {}
    ...
    if (!filePath) {
      ...
      if (isAsset) {
        ...
        if (wantsBr && (await br.exists())) { file = br; encoding = 'br'; filePathUsed = filePath + '.br'; }
        else if (wantsGz && (await gz.exists())) { file = gz; encoding = 'gzip'; filePathUsed = filePath + '.gz'; }
      }
      ...
      if (await file.exists()) {
        ...
        headers['Content-Type'] = map[ext] || base.type || (ext === '.html' ? 'text/html; charset=utf-8' : 'application/octet-stream');
        ...
        if (isAsset) {
          ...
          headers['Cache-Control'] = liveReload ? 'no-store, no-cache, must-revalidate, no-transform' : 'public, max-age=60, stale-while-revalidate=600';
          if (liveReload) { headers['Pragma'] = 'no-cache'; headers['Expires'] = '0'; }
        } else {
          // HTML or other documents
          headers['Cache-Control'] = liveReload ? 'no-store, no-cache, must-revalidate, no-transform' : 'public, max-age=600, stale-while-revalidate=86400';
          if (liveReload) { headers['Pragma'] = 'no-cache'; headers['Expires'] = '0'; }
        }
        // Security headers
