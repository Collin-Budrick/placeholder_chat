...
async function isUp(urlBase) {
  ...
  const check = (t) => new Promise((resolve) => {
    try {
      const u = new URL(t);
...
function getLanHosts() {
  ...
  const preferList = (process.env.LYNX_HOST_PREFER || process.env.WEB_HOST_PREFER || '172.27.,172.,10.,192.168.,192.').split(',').map(s => s.trim()).filter(Boolean);
  const score = (h) => {
...
async function findFirstUpUrl() {
  ...
  for (const h of hosts) {
    for (let p = start; p <= end; p++) candidates.push(`${scheme}://${h}:${p}/`);
  }
  while (Date.now() - startedAt < timeoutMs) {
    // Probe in parallel to minimize delay
    const checks = await Promise.all(candidates.map(async u => ({ url: u, up: await isUp(u) })));
...
(async () => {
  try {
    ...
    const child = spawn(process.execPath, ['scripts/launch-lynx.js', '--url', url], {
      stdio: 'inherit',
      cwd: process.cwd(),
      env: process.env,
    });
