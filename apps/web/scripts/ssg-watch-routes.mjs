...
function bumpVersionFile() {
  try {
    let v = 0;
    try { v = JSON.parse(fs.readFileSync(versionFile, 'utf8')).v | 0; } catch {}
...
async function ensureClientReady(maxWaitMs = 90000) {
  ...
  while (true) {
    try {
      const hasBuild = fs.existsSync(buildDir) && fs.readdirSync(buildDir).some(f => /\.(js|mjs)$/i.test(f));
    ...
    if (Date.now() - start > maxWaitMs) {
      if (DEBUG) console.warn('[ssg-watch] client manifest wait timed out; proceeding');
      return;
    }
...
async function ensureManifestOrBuild() {
  ...
  if (has) return;
  if (DEBUG) console.log('[ssg-watch] q-manifest.json missing — running one-off client build');
  await new Promise((res, rej) => {
    ...
    const c = spawn('bunx', ['vite', 'build', '--logLevel', logLevel], { stdio: 'inherit', cwd: projectRoot, shell: process.platform === 'win32', env: { ...process.env, NO_HTTPS: '1', VITE_WATCH: '0' } });
    c.on('exit', (code) => code === 0 ? res() : rej(new Error('client build (manifest) failed')));
  });
}
...
function syncManifestToServer() {
  try {
    ...
    try { fs.mkdirSync(serverDir, { recursive: true }); } catch {}
    const dst = path.join(serverDir, 'q-manifest.json');
    fs.copyFileSync(src, dst);
    if (DEBUG) console.log('[ssg-watch] synced manifest to server/', path.basename(dst));
  } catch {}
}
...
function listRouteEntryFiles() {
    ...
    try {
      for (const de of fs.readdirSync(dir, { withFileTypes: true })) {
        ...
        if (de.isDirectory()) walk(p);
          ...
          if (/[/\\]index\.(tsx|ts|mdx?|md)$/i.test(p) || /\.(tsx|ts|mdx?|md)$/i.test(p)) {
  ...
  for (const e of out) { if (!byRoute.has(e.route)) byRoute.set(e.route, e.file); }
  return Array.from(byRoute.entries()).map(([route, file]) => ({ route, file }));
}
...
function scheduleGraphRefresh() {
  ...
  graphRefreshTimer = setTimeout(() => { try { routeGraph = buildGraph(); } catch {} }, 500);
}

function spawnSSG(routes) {
  // Ensure SSG builds are one-off and exit; do not set VITE_WATCH=1 here
  writeRoutesFile(routes);
  const env = { ...process.env, SSG_ONLY_ROUTES: routes.join(','), SSG_ROUTES_FILE: routesFile, VITE_WATCH: '0', NO_HTTPS: '1' };
  console.log(`[ssg-watch] Building: ${env.SSG_ONLY_ROUTES}`);
  const logLevel = process.env.VITE_LOG_LEVEL || 'warn';
  return spawn('bunx', ['vite', 'build', '-c', 'adapters/static/vite.config.ts', '--logLevel', logLevel], {
    stdio: 'inherit',
    env,
    cwd: projectRoot,
...
async function initial() {
  ...
  if (!process.env.CLIENT_WATCH) {
    await new Promise((res, rej) => {
      const logLevel = process.env.VITE_LOG_LEVEL || 'warn';
      const c = spawn('bunx', ['vite', 'build', '--logLevel', logLevel], { stdio: 'inherit', cwd: projectRoot, shell: process.platform === 'win32', env: { ...process.env, NO_HTTPS: '1', VITE_WATCH: '1' } });
  ...
  await new Promise((res, rej) => {
    const logLevel = process.env.VITE_LOG_LEVEL || 'warn';
    const c = spawn('bunx', ['vite', 'build', '-c', 'adapters/static/vite.config.ts', '--logLevel', logLevel], { stdio: 'inherit', cwd: projectRoot, shell: process.platform === 'win32', env: { ...process.env, NO_HTTPS: '1', VITE_WATCH: '0' } });
...
function schedule(route) {
  ...
  const run = () => {
    ...
    ensureClientReady().then(async () => {
      ...
      let finished = false;
      let timer = setTimeout(() => finalize('timeout'), 120000);
...
function watchDir(dir) {
  try {
    ...
    try { if (process.platform === 'win32') opts.recursive = false; } catch {}
    fs.watch(dir, opts, (event, filename) => {
      if (!filename) return;
      const abs = path.join(dir, filename.toString());
      if (isDuplicateEvent(abs)) return;
      if (DEBUG) console.log('[ssg-watch] fs.watch', event, path.relative(projectRoot, abs));
      ...
      // Non-route project source: map to impacted routes
        ...
        if (impacted.length) {
          console.log(`[ssg-watch] Change in ${path.relative(projectRoot, abs)} -> ${impacted.join(',')}`);
      ...
      for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
        ...
        if (entry.isDirectory()) {
          ...
          if (nowT > prev.t || nowS !== prev.s) {
            ...
            if (DEBUG) console.log('[ssg-watch] poll change', path.relative(projectRoot, p));
            ...
            if (route) schedule(route);
              ...
              if (impacted.length) {
                console.log(`[ssg-watch] Change in ${path.relative(projectRoot, p)} -> ${impacted.join(',')}`);
                for (const r of impacted) schedule(r);
              }
              scheduleGraphRefresh();
            }
    ...
    (function seed(dir){
      for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
        ...
        if (entry.isDirectory()) seed(p);
          ...
          try { const st = fs.statSync(p); mtimes.set(p, { t: st.mtimeMs || st.mtime?.getTime?.() || Date.now(), s: st.size || 0 }); } catch {}
        }
      }
    })(srcRoot);
  } catch {}
